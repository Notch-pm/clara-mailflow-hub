...
const corsHeaders = {
  ...
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type, x-org-id, x-auth-hook-secret, webhook-secret, x-supabase-client-platform, x-supabase-client-platform-version, x-supabase-client-runtime, x-supabase-client-runtime-version",
};
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Configure the secret value generated by Supabase Dashboard
    // (Auth → Hooks → Send Email hook) as AUTH_HOOK_SECRET edge function secret.
    // The Supabase-provided value starts with "v1,whsec_..." — strip the
    // "v1,whsec_" prefix when storing if needed; the standardwebhooks lib
    // accepts the base64 portion only.
