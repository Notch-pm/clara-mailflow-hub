-- Ajoute un filtre "a envoyé un courrier entre [date début] et [date fin]"
-- à la recherche d'usagers (présence d'au moins un courrier entrant reçu
-- dans la période, indépendamment du nombre total de courriers entrants).

-- On ajoute des paramètres : DROP + CREATE plutôt que CREATE OR REPLACE,
-- pour éviter de créer une surcharge ambiguë (Postgres ne remplace une
-- fonction in-place que si la liste de paramètres reste strictement identique).
DROP FUNCTION IF EXISTS public.search_usagers(uuid, text, uuid[], int, int, int);
...
REVOKE EXECUTE ON FUNCTION public.search_usagers(uuid, text, uuid[], int, int, date, date, int) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.search_usagers(uuid, text, uuid[], int, int, date, date, int) TO authenticated;
