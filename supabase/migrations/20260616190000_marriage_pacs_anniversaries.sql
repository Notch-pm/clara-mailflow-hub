-- Note : l'ajout de la valeur d'enum 'divorce' a été appliqué séparément
-- (ALTER TYPE ... ADD VALUE ne peut pas partager une transaction avec
-- d'autres DDL qui référencent potentiellement la nouvelle valeur).

-- 1. Dates de mariage / PACS — affichées côté UI seulement si family_status
-- correspondant (mariage / pacse), mais conservées même après passage en
-- "divorce" (historique), cf. filtre anniversaires de mariage ci-dessous.
ALTER TABLE public.usagers
  ADD COLUMN IF NOT EXISTS marriage_date date,
  ADD COLUMN IF NOT EXISTS pacs_date date;

-- 2. search_usagers : ajout des filtres "grands anniversaires de mariage"
-- et "grands anniversaires" (âge). DROP + CREATE car on ajoute des paramètres.
DROP FUNCTION IF EXISTS public.search_usagers(uuid, text, uuid[], int, int, date, date, int);
...
REVOKE EXECUTE ON FUNCTION public.search_usagers(uuid, text, uuid[], int, int, date, date, int[], int[], int) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.search_usagers(uuid, text, uuid[], int, int, date, date, int[], int[], int) TO authenticated;
